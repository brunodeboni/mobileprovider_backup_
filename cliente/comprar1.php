...
<body>
    <div id="container">
        ...
        <form id="form_comprar" method="post" action="" ng-controller="Comprar">
            ...
            $email = $client['email'];
            $licenca_instalacao = $client['licenca_instalacao']; //retorna FALSE se não houver
		...
		<select id="dias_licenca" name="dias_licenca" class="block" ng-model="dias_licenca">
			<option value="0" ng-show="dias_faltando>0&&licenca=='Licença por Usuário'">{{dias_faltando}} dias</option>
        ...
        <div ng-show="licenca=='Licença por Usuário'">
			<span id="qnt">Quantidade de Usuários (novos):</span>
		...
		<div id="valor-licenca">Licença {{msg_licenca}}: 
		    R$ {{licencaUso}} (válido por {{dias_total}} dias)</div>
		<div id="igual"> = </div>
...
function Comprar($scope) {
    ...
    $scope.calculaLicenca = function() {
		$scope.dias_total = parseInt($scope.dias_licenca) + parseInt($scope.dias_faltando);
		$scope.quantidade = parseInt($scope.quantidade_old) + parseInt($scope.quantidade);
		
		if ($scope.licenca === "Licença por Usuário") {
			$scope.itemDescription2 = 'Licença para '+ $scope.quantidade +' Usuário(s) (válido por '+ $scope.dias_total +' dias)';
			...
			var valor_licenca_plano_total = valor_licenca_plano_usuario * (parseInt($scope.quantidade) + parseInt($scope.quantidade_old)); //Valor total da licença para o plano

			if ($scope.dias_faltando > 0) {
				var valor_licenca_dias_faltando = $scope.quantidade * $scope.dias_faltando * valor_licenca_usuario_dia; //Valor da licença para todos os usuário nos dias faltando
				var valor_licenca_total = parseFloat(valor_licenca_dias_faltando) + parseFloat(valor_licenca_plano_total); //Valor total é o valor do plano + valor dos dias faltando
			}else {
			...
			$scope.itemDescription2 = 'Licença Ilimitada (válido por '+ $scope.dias_total +' dias)';
			$scope.quantidade = 1;
			$scope.msg_licenca = 'Ilimitada';
			...
			if ($scope.dias_faltando > 0) {
				var valor_licenca_dias_faltando = $scope.dias_faltando * valor_licenca_dia; //Valor da licenÃ§a para os dias faltando
				var valor_licenca_total = parseFloat(valor_licenca_dias_faltando) + parseFloat(valor_licenca_plano); //Valor total é o valor do plano + valor dos dias faltando
			}else {
		...
		if ($scope.licencaInstalacao === 0) {
			$scope.licencaTotal = (parseFloat(valor_licenca_total) + 500).replace('.', ','); //Valor das duas licenças somadas
		}else {
